...
JOB_YAML=$(mktemp)
CONFIG_YAML=${REPO_ROOT_DIR}/config/staging/prow/core/config.yaml
JOB_CONFIG_YAML=${REPO_ROOT_DIR}/config/staging/prow/jobs
...
make -C "${REPO_ROOT_DIR}/config/staging" get-cluster-credentials
kubectl apply -f ${JOB_YAML}
#make -C "${REPO_ROOT_DIR}/config/staging" unset-cluster-credentials
